...
支持接入 OpenAI API 兼容的大语言模型，提供 AI 辅助编辑能力。
...
- **系统提示词**：管理多个系统提示词，可切换使用
- **模式设置**：
  - 启用 AI 面板：在编辑器工具栏显示 AI 按钮
  - 允许读取当前文档：AI 可访问当前文档内容，并支持 Agent 模式
- **同步设置**：将配置同步到服务器，支持多设备使用

### 使用模式

- **Chat 模式**：普通对话模式，可配合文档上下文使用
- **Agent 模式**：AI 输出结构化修改建议，可一键应用到文档（需开启「允许读取当前文档」），支持流式实时解析显示
